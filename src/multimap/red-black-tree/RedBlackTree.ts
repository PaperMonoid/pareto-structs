...
export default class RedBlackTree<K, V> implements MultiMap<K, V> {
  ...
  static create<K, V>(
    ...
    return new RedBlackTree<K, V>(comparator, equals, root, count);
  }

  setRoot(node: Node<K, V>): RedBlackTree<K, V> {
  ...
  setCount(count: number): RedBlackTree<K, V> {
    return new RedBlackTree<K, V>(
      this.comparator,
      this.equals,
      this.root,
  ...
  private addNode(newNode: Node<K, V>, node: Node<K, V>): Node<K, V> {
    if (!newNode) {
      ...
      if (comparison > 0) {
        return node
          .setRight(this.addNode(newNode, node.right))
          .fixRedViolation();
  ...
  put(key: K, value: V): MultiMap<K, V> {
    const array = [value];
    const node = this.addNode(new Node<K, V>(key, array, Color.Red), this.root);
  ...
  private replaceNode(
    ...
    if (!newNode) {
      ...
      if (comparison > 0) {
        ...
        return [node.setRight(right).fixRedViolation(), diff];
      } else if (comparison < 0) {
  ...
  replace(key: K, values: Iterable<V>): MultiMap<K, V> {
    ...
    const [node, diff] = this.replaceNode(
      new Node<K, V>(key, array, Color.Red),
      this.root
    );
  ...
  private updateRemoveNode(
    ...
    if (comparison == 0) {
      ...
      if (values.length == 0) {
        ...
        return Optional.ofValue([node.setValues(values), false]);
      }
    } else if (comparison > 0) {
      return this.updateRemoveNode(key, value, node.right).map(
        ([removed, fix]) => node.setRight(removed).fixRightViolation(fix)
      );
    } else {
      return this.updateRemoveNode(key, value, node.left).map(
        ([removed, fix]) => node.setLeft(removed).fixLeftViolation(fix)
      );
    }
  }
  ...
  private removeNode(
    key: K,
    node: Node<K, V>
  ): [Optional<[Node<K, V>, boolean]>, number] {
    if (!node) {
  ...
  removeAll(key: K): MultiMap<K, V> {
    ...
    return removed
      .map(([replaced, fix]) =>
        tree.setRoot(replaced).setCount(tree.count - diff)
      )
      .orValue(this);
  }
  ...
  clear(): MultiMap<K, V> {
    return new RedBlackTree<K, V>(this.comparator, this.equals);
  }
  ...
  iterator(key: K): ListIterator<[K, V]> {
    return new RedBlackTreeListIterator<K, V>(this, key);
  }
}
